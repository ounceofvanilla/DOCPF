      ...
      case 'bitbucket':
         ...
         withCredentials([usernamePassword(credentialsId: "${pipelineParams.BITBUCKET_CREDENTIALS}", usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASSWORD')]) 
         {
            ...
            if (isUnix()) {
               sh ("""
                  set +x
                  git -c user.name=""" + '${GIT_USER}' + """ -c user.email=Do-Not-Reply@local.com tag -a ${pipelineParams.RELEASE_TAG} ${GIT_COMMIT} -m "Tag ${pipelineParams.RELEASE_TAG} Created"
                  git -c user.name=""" + '${GIT_USER}' + """ -c user.email=Do-Not-Reply@local.com push https://""" + '${GIT_USER}:${URL_ENCODED_GIT_PASSWORD}' + """@${repoUrl} --tags
                  """)

                  echo """
                  git -c user.name=${GIT_USER} -c user.email=Do-Not-Reply@local.com tag -a ${pipelineParams.RELEASE_TAG} ${GIT_COMMIT} -m "Tag ${pipelineParams.RELEASE_TAG} Created"
                  git -c user.name=${GIT_USER} -c user.email=Do-Not-Reply@local.com push https://""" + '${GIT_USER}:*****' + """@${repoUrl} --tags
                  """
