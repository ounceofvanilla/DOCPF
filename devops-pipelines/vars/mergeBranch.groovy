      ...
      switch(pipelineParams.SOURCE_CODE_TOOL) {
         case 'bitbucket':
            ...
            withCredentials([usernamePassword(credentialsId: 'BITBUCKET_CREDENTIALS', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASSWORD')]) 
            {
               ...
               "${osCommand}" ("""
                  git -c user.name=${GIT_USER} -c user.email=Do-Not-Reply@local.com checkout ${MERGE_TO_BRANCH}
                  git -c user.name=${GIT_USER} -c user.email=Do-Not-Reply@local.com pull --no-ff https://${GIT_USER}:${GIT_PASSWORD}@${repoUrl} ${MERGE_FROM_BRANCH}
                  git -c user.name=${GIT_USER} -c user.email=Do-Not-Reply@local.com push https://${GIT_USER}:${GIT_PASSWORD}@${repoUrl} ${MERGE_TO_BRANCH}
                  """)
            }
            break
         default:
